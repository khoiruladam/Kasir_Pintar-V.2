...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($username) || empty($password) || empty($level)) {
        tampilkanUI('error', 'Data Belum Lengkap', 'Pastikan kolom username, password, dan level jabatan sudah terisi.', 'javascript:history.back()');
    }
    ...
    try {
        ...
        tampilkanUI('success', 'Pengguna Aktif!', "Akses sistem untuk <b>" . htmlspecialchars($username) . "</b> berhasil dibuat.", $url_kembali . '?pesan=sukses');

    } catch (PDOException $e) {
        if ($e->getCode() == '23000') {
            tampilkanUI('error', 'Username Terpakai!', "Username <b>{$username}</b> sudah digunakan oleh orang lain. Silakan pilih nama lain.", 'javascript:history.back()');
        } else {
