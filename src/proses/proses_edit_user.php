...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($id_user == 0 || empty($username) || empty($level)) {
        tampilkanUI('error', 'Data Belum Lengkap', 'Pastikan semua kolom terisi dengan benar.', 'javascript:history.back()');
    }

    try {
        $query = "UPDATE user SET Username = ?, Level = ? WHERE UserID = ?";
        $stmt = $koneksi->prepare($query);
        $stmt->execute([$username, $level, $id_user]);

        tampilkanUI('success', 'Akses Diperbarui!', "Data pengguna <b>" . htmlspecialchars($username) . "</b> berhasil diubah.", $url_kembali);

    } catch (PDOException $e) {
        if ($e->getCode() == '23000') {
            tampilkanUI('error', 'Username Terpakai!', "Username <b>{$username}</b> sudah digunakan oleh pengguna lain.", 'javascript:history.back()');
        } else {
    ...
    header('Location: ../resources/views/admin/user.php');
    exit();
}
?>