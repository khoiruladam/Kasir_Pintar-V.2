...
if ($level_user != 'petugas' && $level_user != 'kasir') {
    tampilkanUI('error', 'Akses Ditolak!', 'Anda tidak memiliki hak untuk menghapus data member.', $url_kembali);
}
...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    if ($id_pelanggan === 1) {
        tampilkanUI('error', 'Aksi Ditolak!', 'Data Pelanggan Umum adalah pengaturan sistem bawaan yang tidak boleh dihapus.', $url_kembali);
    }

    try {
        $stmt = $koneksi->prepare("DELETE FROM pelanggan WHERE PelangganID = ?");
        $stmt->execute([$id_pelanggan]);

        if ($stmt->rowCount() > 0) {
            tampilkanUI('success', 'Data Dihapus!', 'Data member telah berhasil dihapus dari sistem kasir.', $url_kembali);
        } else {
            tampilkanUI('error', 'Gagal Menghapus', 'Data member tidak ditemukan di database.', $url_kembali);
        }

    } catch (PDOException $e) {
        if ($e->getCode() == '23000') {
            tampilkanUI('error', 'Gagal Dihapus!', 'Member ini sudah pernah melakukan transaksi. Hapus riwayat belanjanya terlebih dahulu untuk melindungi laporan keuangan.', $url_kembali);
        } else {
