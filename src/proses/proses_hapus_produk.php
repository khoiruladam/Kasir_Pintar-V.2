...
if (isset($_GET['id'])) {
    $id_produk = $_GET['id'];
    $url_kembali = '../resources/views/admin/produk.php';
    ...
    try {
        ...
        if ($stmt->rowCount() > 0) {
            tampilkanUI('success', 'Produk Dihapus!', 'Data produk berhasil dihilangkan dari sistem.', $url_kembali . '?pesan=hapus_sukses');
        } else {
            tampilkanUI('error', 'Gagal Menghapus', 'Data produk tidak ditemukan atau sudah dihapus sebelumnya.', $url_kembali);
        }

    } catch (PDOException $e) {
        if ($e->getCode() == '23000') {
            tampilkanUI('error', 'Tidak Bisa Dihapus!', 'Produk ini tidak bisa dihapus karena sudah tercatat dalam riwayat transaksi kasir.', $url_kembali);
        } else {
            $pesan_error = htmlspecialchars($e->getMessage());
            tampilkanUI('error', 'Sistem Error', $pesan_error, $url_kembali);
        }
    }
} else {
